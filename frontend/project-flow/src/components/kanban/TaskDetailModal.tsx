...
import { useCreateComment, useCreateSubtask, useToggleSubtask, useDeleteSubtask, useGenerateAiSubtasks, useWorkspaces, useDeleteTask, useAddTaskLabel, useRemoveTaskLabel, useAddTaskAssignee, useRemoveTaskAssignee } from "@/hooks/useApi";
...
export default function TaskDetailModal({ task, open, onClose, onUpdate, members, labels }: Props) {
  ...
  const handleAddSubtask = async () => {
    ...
    try {
        await createSubtaskMutation.mutateAsync({ taskId: task.id, title: newSubtask.trim() });
        setNewSubtask("");
    } catch (err) {}
  };
  ...
  const handleToggleSubtask = async (id: string) => {
    try {
        await toggleSubtaskMutation.mutateAsync({ subtaskId: id });
    } catch (err) {}
  };
  ...
  const handleAddComment = async () => {
    ...
    try {
        await createCommentMutation.mutateAsync({ taskId: task.id, text: newComment.trim() });
        setNewComment("");
    } catch (err) {}
  };
  ...
  const handleAddLabel = async (labelId: string) => {
    if (task.labels?.find((l: any) => l.id === labelId)) return;
    try {
        await addLabelMutation.mutateAsync({ taskId: task.id, labelId });
    } catch (err) {}
  };

  const handleRemoveLabel = async (labelId: string) => {
    try {
        await removeLabelMutation.mutateAsync({ taskId: task.id, labelId });
    } catch (err) {}
  };

  const handleAddAssignee = async (userId: string) => {
    if (task.assignees?.find((a: any) => a.id === userId)) return;
    try {
        await addAssigneeMutation.mutateAsync({ taskId: task.id, userId });
    } catch (err) {}
  };

  const handleRemoveAssignee = async (userId: string) => {
    try {
        await removeAssigneeMutation.mutateAsync({ taskId: task.id, userId });
    } catch (err) {}
  };
  ...
  const handleAiBreakdown = async () => {
    try {
        setAiLoading(true);
        await generateAiMutation.mutateAsync(task.id);
        setAiLoading(false);
    } catch (err) {
        setAiLoading(false);
    }
  ...
  const keepAiSuggestion = async (suggestion: { id: string; title: string }) => {
    try {
        await createSubtaskMutation.mutateAsync({ taskId: task.id, title: suggestion.title });

...
import { LABELS as INITIAL_LABELS, type Label } from "@/types/kanban";
...
export default function LabelManagementModal({ open, onClose }: Props) {
    ...
    const handleSaveEdit = () => {
        ...
        setLabels(labels.map(l => l.id === editingId ? { ...l, name: newName.trim(), color: newColor } : l));
        setEditingId(null);
        setNewName("");
                        ...
                        <div key={label.id} className="flex items-center gap-3 p-2 rounded-lg border border-border/50 bg-secondary/10 group">
                            {editingId === label.id ? (
                                <div className="flex-1 flex items-center gap-2">
                                    ...
                                    <button onClick={handleSaveEdit} className="p-1 rounded hover:bg-primary/20 text-primary">
                                ...
                                <>
                                    ...
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 sidebar-transition">
                                        <button onClick={() => startEdit(label)} className="p-1 rounded text-muted-foreground hover:text-foreground hover:bg-accent">
                                            <Plus className="h-3.5 w-3.5 rotate-45" /> {/* Use as an edit icon shorthand or simple styling */}
                                        </button>
                                        <button onClick={() => handleDelete(label.id)} className="p-1 rounded text-muted-foreground hover:text-destructive hover:bg-destructive/10">
                    ...
                    <form onSubmit={handleAdd} className="mt-6 p-4 rounded-lg bg-secondary/20 border border-primary/20 space-y-4">
                        <div className="flex items-center gap-3">
                            <input
                                ...
                                value={newName}
                                onChange={(e) => setNewName(e.target.value)}
                                className="flex-1 bg-background border border-border rounded px-3 py-1.5 text-sm focus:ring-1 focus:ring-primary outline-none"
                                autoFocus
                            />
                        </div>
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            <button type="submit" className="px-3 py-1 bg-primary text-primary-foreground text-xs font-medium rounded hover:bg-primary/90">Add Label</button>
                        </div>
                    </form>
                ) : (
                    <button
